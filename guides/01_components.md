...
- **4x 1.2V rechargeable AA batteries** (with series holder and switch)
  - to provide 4.8V to the ESP32-CAMs, one for each eye.
...
- **1x 2-pin switch**
  - to easily turn the robot on and off, noting that another switch would be needed if one wasn't available in the battery holder (since ESP32-CAMs use a separate power line to avoid voltage spikes during motor and servo use).
...
And finally, the following are also used:

- **1x Computer**
- **1x Router**
  - for internet access and/or phone with hotspot, to connect 2x ESP32-CAMs, ESP32-WROVER, and computer to the same network.
