...
IPAddress staticIP2(172, 20, 10, 10);                   // Static IP for hotspot
...
void handleCameraConfig(AsyncWebServerRequest *request) {
    ...
    if (request->method() == HTTP_POST) {
        ...
        config.pin_xclk = 0;        // xclk (external clock) input
        config.pin_pclk = 22;       // pixel clock
        config.pin_vsync = 25;
        config.pin_href = 23;
        config.pin_sscb_sda = 26;   // serial data line
        config.pin_sscb_scl = 27;   // serial clock line
